...
('Andrew Cook', 'Sales', 68000.50, '2020-04-13'),
...
----------------------------------------------------------------------
-- 1. Retrieve all employees whose salary is greater than the highest salary of the HR department.
SELECT MAX(salary) as max_salary FROM employees
WHERE department_name = 'HR';

SELECT * FROM employees
WHERE salary > (SELECT MAX(salary) as max_salary FROM employees WHERE department_name = 'HR');
----------------------------------------------------------------------
...
----------------------------------------------------------------------
-- FOR PRACTICE PURPOSE //NEED some changes
SELECT department_name, SUM(salary) FROM employees GROUP BY(department_name);

SELECT * FROM(SELECT department_name, SUM(salary) as sum_salary FROM employees GROUP BY(department_name))

SELECT * FROM employees WHERE department_name in (SELECT department_name FROM employees WHERE department_name LIKE('%R%') )
----------------------------------------------------------------------